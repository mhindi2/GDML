...
def createBox(
    ...
    for prop in ["x", "y", "z"]:
        expr = GDMLShared.getSheetExpression(solid, prop)
        if expr is not None:
            mycube.setExpression(prop, expr)
...
def createElcone(part, solid, material, colour, px, py, pz, rot, displayMode, solidName = None):
    ...
    for prop in ["dx", "dy", "zmax", "zcut"]:
        expr = GDMLShared.getSheetExpression(solid, prop)
        if expr is not None:
            myelcone.setExpression(prop, expr)
...
def createEllipsoid(
    ...
    for prop in ["ax", "by", "cz", "zcut1", "zcut2"]:
        expr = GDMLShared.getSheetExpression(solid, prop)
        if expr is not None:
            myelli.setExpression(prop, expr)
...
def createEltube(part, solid, material, colour, px, py, pz, rot, displayMode, solidName = None):
    ...
    for prop in ["dx", "dy", "dz" ]:
        expr = GDMLShared.getSheetExpression(solid, prop)
        if expr is not None:
            myeltube.setExpression(prop, expr)
...
def createGenericPolycone(
    ...
    for prop in ["startphi", "deltaphi"]:
        expr = GDMLShared.getSheetExpression(solid, prop)
        if expr is not None:
            mypolycone.setExpression(prop, expr)
...
def createGenericPolyhedra(
    ...
    for prop in ["startphi", "deltaphi", "numsides"]:
        expr = GDMLShared.getSheetExpression(solid, prop)
        if expr is not None:
            mypolyhedra.setExpression(prop, expr)
...
def createPara(
    ...
    for prop in ["x", "y", "z", "alpha", "theta", "phi"]:
        expr = GDMLShared.getSheetExpression(solid, prop)
        if expr is not None:
            mypara.setExpression(prop, expr)
...
def createHype(
    ...
    for prop in ["rmin", "rmax", "z", "inst", "outst"]:
        expr = GDMLShared.getSheetExpression(solid, prop)
        if expr is not None:
            myhype.setExpression(prop, expr)
...
def createParaboloid(
    ...
    for prop in ["rlo", "rhi", "dz"]:
        expr = GDMLShared.getSheetExpression(solid, prop)
        if expr is not None:
            myparaboloid.setExpression(prop, expr)
...
def createScaledSolid(
    ...
    # TODO scaling parameters as expressions
...
def createSphere(
    ...
    for prop in ["rmin", "rmax", "startphi", "deltaphi", "starttheta", "deltatheta"]:
        expr = GDMLShared.getSheetExpression(solid, prop)
        if expr is not None:
            mysphere.setExpression(prop, expr)
...
def createTorus(
    ...
    for prop in ["rmin", "rmax", "rtor", "startphi", "deltaphi"]:
        expr = GDMLShared.getSheetExpression(solid, prop)
        if expr is not None:
            mytorus.setExpression(prop, expr)
...
def createTrap(part, solid, material, colour, px, py, pz, rot, displayMode, solidName=None):
    ...
    # Our GDMLTrap does not have alpha1, alpha2, just alpha
    for prop in ["z", "x1", "x2", "x3", "x4", "y1", "y2", "theta", "phi", "alpha"]:
        expr = GDMLShared.getSheetExpression(solid, prop)
        if expr is not None:
            mytrap.setExpression(prop, expr)
...
def createTrd(
    ...
    for prop in ["z", "x1", "x2", "y1", "y2"]:
        expr = GDMLShared.getSheetExpression(solid, prop)
        if expr is not None:
            mytrd.setExpression(prop, expr)
...
def createTwistedbox(
    ...
    for prop in ["x", "y", "z", "PhiTwist"]:
        expr = GDMLShared.getSheetExpression(solid, prop)
        if expr is not None:
            mypart.setExpression(prop, expr)
...
def createTwistedtrap(
    ...
    for prop in ["z", "x1", "x2", "x3", "x4", "y1", "y2", "Theta", "Phi", "Alph"]:
        expr = GDMLShared.getSheetExpression(solid, prop)
        if expr is not None:
            mytrap.setExpression(prop, expr)
...
def createTwistedtrd(
    ...
    for prop in ["z", "x1", "x2", "y1", "y2", "PhiTwist"]:
        expr = GDMLShared.getSheetExpression(solid, prop)
        if expr is not None:
            mytrd.setExpression(prop, expr)
...
def createTwistedtubs(
    ...
    for prop in ["zlen", "endinnerrad", "endouterrad", "twistedangle", "phi"]:
        expr = GDMLShared.getSheetExpression(solid, prop)
        if expr is not None:
            mypart.setExpression(prop, expr)
...
def createTube(part, solid, material, colour, px, py, pz, rot, displayMode, solidName=None):
    ...
    for prop in ["rmin", "rmax", "z", "startphi", "deltaphi"]:
        expr = GDMLShared.getSheetExpression(solid, prop)
        if expr is not None:
            mytube.setExpression(prop, expr)
...
def createCutTube(part, solid, material, colour, px, py, pz, rot, displayMode, solidName=None):
    ...
    for prop in ["rmin", "rmax", "z", "startphi", "deltaphi",
        ...
        expr = GDMLShared.getSheetExpression(solid, prop)
        if expr is not None:
            mycuttube.setExpression(prop, expr)
...
def createParameterisedTube(
    ...
    for prop in ["InR", "OutR", "hz", "StartPhi", "DeltaPhi"]:
        expr = GDMLShared.getSheetExpression(solid, prop)
        if expr is not None:
            mytube.setExpression(prop, expr)
...
def createParameterisedPolyhedra(
    ...
    for prop in ["startPhi", "openPhi", "z", "numSides"]:
        expr = GDMLShared.getSheetExpression(solid, prop)
        if expr is not None:
            mypolyhedra.setExpression(prop, expr)
...
def processReplica(importFlag, doc, vol, volDict, parent, phylvl, displayMode):
    from .GDMLObjects import getAngleRad
    from .GDMLObjects import ViewProvider
    # vol is an xml <volume item, containing a <replicavol
    # parent is an exisiting App::Part  that already contains the
    # the mother solid 
    # If the mother volume was referenced in a physvol' the parent App::Part
    # would already have been created and the mother volume already placed
    # in the corresponding App::Part of the mother volume.
    # Here we need to create a NEW App::Part and place either an array
    # of the replicated solid in it (in case of ortho or phi replicas) or
    # and assembly of replicated solids in case of a "rho" replica.
    #
    # If the mother volume is NOT placed via a physVol placement, but is
    # is rather itself a replica, then we have to create the APP:Part here
    ...
    volRef = GDMLShared.getRef(replicavol, "volumeref")
    part = parent.newObject("App::Part", volRef)
    if volRef is None:
        print("Error - volume reference of replicavol not found")
        return None

    baseVol = structure.find("volume[@name='%s']" % volRef)
    replicatedSolid = processVol(importFlag, doc, baseVol, volDict, part, phylvl, displayMode)
    ...
    if "phi" in direction.attrib:
        ...
        offset *= mul
        part.addObject(replicatedSolid)
        rmin = replicatedSolid.rmin
        rmax = replicatedSolid.rmax
        if FreeCAD.GuiUp:
            # set ViewProvider before setDisplay
            ViewProvider(replicatedSolid.ViewObject)
            setDisplayMode(replicatedSolid, 4)
        for i in range(1, number):  # firt copy is already placed above
            copy = doc.copyObject(replicatedSolid, True)
            # copy.Proxy = copy
            rmin = rmax
            rmax = rmin + width
            copy.rmax = rmax
        ...
        array = Draft.make_array(part, xvector, yvector, zvector,
                                 xnum, ynum, znum)
        parent.addObject(array)
...
def processVol(importFlag, doc, vol, volDict, parent, phylvl, displayMode):
    ...
    if replicavol is not None:
        processReplica(importFlag, doc, vol, volDict, parent, phylvl, displayMode)
